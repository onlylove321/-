...
public class LoginController extends BaseController {
	 ...
	 * 请求登录，验证用户
	 */
	@RequestMapping(value = "/login_login", produces = "application/json;charset=UTF-8")//produces 指定返回的内容类型
	@ResponseBody
			...
			if (null == code || "".equals(code)) {
				...
				if (Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)) {
					...
					if (pd != null) {
						...
						session.setAttribute("loginUser", user.getUSERNAME());  //保存登录用户
						// shiro加入身份验证
